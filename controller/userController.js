...
export const register = catchAsyncErrors(async (req, res, next) => {
  ...
  // Access the PDF file
  const pdfFile = req.files.resume;

  // Upload the PDF to Cloudinary
  const result = await cloudinary.uploader.upload(pdfFile.tempFilePath, {
    resource_type: "raw", // Specify raw since it's a PDF
    public_id: pdfFile.name.split(".")[0], // Optional: Use the file name as public_id
  ...
  const user = await User.create({
    ...
    resume: {
      public_id: result.public_id, // Set your cloudinary public_id here
      url: result.secure_url, // Set your cloudinary secure_url here
    },
  });
...
export const getUserForPortfolio = catchAsyncErrors(async (req, res, next) => {
  const id = "66f9364a98d89f6bb8b77d84";
